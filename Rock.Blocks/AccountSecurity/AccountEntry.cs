...
using Rock.Data;
using Rock.Enums.Blocks.AccountSecurity.AccountEntry;
using Rock.Model;
using Rock.Security;
using Rock.Security.Authentication;
using Rock.ViewModels.Blocks.AccountSecurity.AccountEntry;
...
namespace Rock.Blocks.AccountSecurity
{
    ...
    public class AccountEntry : RockObsidianBlockType
    {
        ...
        public BlockActionResult ForgotUsername( AccountEntryForgotUsernameRequestBag bag )
        {
            using ( var rockContext = new RockContext() )
            {
                ...
                var mergeObjects = GetMergeFields(
                    ...
                    {
                        { "ConfirmAccountUrl", PathHelper.GetRootPath() + url.TrimStart( new char[] { '/' } ) },
                        { "Results",
                            new Dictionary<string, object>[]
                            {
                                new Dictionary<string, object>
                ...
                {
                    AppRoot = "/",
                    ThemeRoot = PathHelper.ResolveRockUrl( "~~/", null ),
        ...
        private AccountEntryInitializationBox GetInitializationBox( string encryptedStateOverride = null )
        {
            ...
            var passwordlessLoginState = Encryption.DecryptString( passwordlessLoginStateString ).FromJsonOrNull<PasswordlessAuthenticationState>();
        ...
        private bool IsFromPasswordlessAuthentication( AccountEntryRegisterRequestBox box, out PasswordlessAuthenticationState state )
        {
            ...
            state = Encryption.DecryptString( box?.State ).FromJsonOrThrow<PasswordlessAuthenticationState>();
            return state != null;
        }
        ...
        /// Tries to finalize a passwordless authentication session if one exists.
        /// </summary>
        /// <param name="bag">The registration request bag.</param>
        /// <param name="person">The registered person.</param>
        /// <param name="rockContext">The context.</param>
        /// <returns><c>true</c> if a passwordless authentication session was finalized; otherwise, <c>false</c>.</returns>
        private bool IsPasswordlessAuthenticationValid( Person person, PasswordlessAuthenticationState passwordlessAuthenticationState, RockContext rockContext, out RemoteAuthenticationSession remoteAuthenticationSession )
        {
            var remoteAuthenticationSessionService = new RemoteAuthenticationSessionService( rockContext );
        ...
        private BlockActionResult RegisterExistingPerson( Person person, AccountEntryRegisterRequestBox box, AccountEntryInitializationBox config, RockContext rockContext )
        {
            ...
            {
                if ( !IsPasswordlessAuthenticationValid( person, passwordlessAuthenticationState, rockContext, out var remoteAuthenticationSession ) )
                {
                ...
                if ( userLogin == null )
                {
                    ...
                    // It will be a confirmed account if using passwordless authentication with an email (IsEmailHidden == true).
                    userLogin = CreatePasswordlessUserLogin( person, config.IsEmailHidden, username, rockContext );
                    accountCreated = true;
                }
        ...
        private void SendAccountCreatedEmail( UserLogin userLogin, Person person )
        {
            try
            {
                ...
                emailMessage.ThemeRoot = PathHelper.ResolveRockUrl("~~/", null);
