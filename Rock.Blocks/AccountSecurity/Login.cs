...
using Rock.Data;
using Rock.Enums.Blocks.AccountSecurity.Login;
using Rock.Model;
using Rock.Security;
using Rock.Security.Authentication;
using Rock.ViewModels.Blocks.AccountSecurity.Login;
using Rock.ViewModels.Utility;
using Rock.Web.Cache;
using Rock.Web.UI.Controls;
using Rock.Web.Utilities;
using static Rock.Security.Authentication.PasswordlessAuthentication;

namespace Rock.Blocks.AccountSecurity
{
    /// <summary>
    ...
    public class Login : RockObsidianBlockType
    {
        ...
        public override object GetObsidianBlockInitialization()
        {
            ...
            var secondaryAuthenticationProviders = GetSecondaryAuthenticationComponents(); 
        ...
        public BlockActionResult CredentialLogin( CredentialLoginRequestBag bag )
        {
            ...
            using ( var rockContext = new RockContext() )
            {
                ...
                Security.Authorization.SetAuthCookie( userLogin.UserName, bag.RememberMe, isImpersonated: false );
        ...
        public BlockActionResult PasswordlessLoginVerify( PasswordlessLoginVerifyRequestBag bag )
        {
            using ( var rockContext = new RockContext() )
            {
                ...
                var result = passwordlessAuthentication.VerifyOneTimePasscode(
                    new VerifyOneTimePasscodeOptions
                    {
                        Code = bag.Code,
        ...
        public BlockActionResult RemoteLoginStart( RemoteLoginStartRequestBag bag )
        {
            ...
            {
                // The API consumer sent an non-remote authentication type.
                // Show an error if this unlikely event occurs.
                return ActionBadRequest( "Please use an external authentication provider" );
            }

            var loginUri = authenticationComponent.GenerateLoginUrl( GetCurrentPageUrl(), GetRedirectUrlAfterLogin() );
            ...
            {
                RedirectUrl = loginUri.AbsoluteUri
            } );
        }
        ...
        {
            return $"{PathHelper.GetRootPath()}page/{PageCache.Id}";
        }
        ...
        /// <returns></returns>
        private static List<ExternalAuthenticationButtonBag> GetExternalAuthProviderButtons( List<NamedComponent<AuthenticationComponent>> externalAuthProviders )
        {
            ...
            foreach ( var component in externalAuthProviders )
            {
                externalAuthenticationButtons.Add( new ExternalAuthenticationButtonBag
                {
                    ...
                    ImageUrl = PathHelper.ResolveRockUrl( component.Component.ImageUrl() ),
        ...
        /// <summary>
        /// Gets all secondary authentication components.
        /// </summary>
        ...
        private List<NamedComponent<AuthenticationComponent>> GetSecondaryAuthenticationComponents()
        {
        ...
        private void LogInWithExternalAuthProviderIfNeeded( LoginInitializationBox box, List<NamedComponent<AuthenticationComponent>> externalAuthProviders )
        {
            ...
            {
                if ( !authProvider.IsFromExternalAuthentication( RequestContext.QueryString ) )
                {
                    continue;
                }

                var isAuthenticationSuccessful = authProvider.Authenticate( GetCurrentPageUrl(), RequestContext.QueryString, out var username, out var returnUrl );

                if ( !isAuthenticationSuccessful )
                {
                    // Although authentication failed for the current auth provider,
                    // we should continue to try others as the IsFromExternalAuthentication return value may have been a false positive.
                    continue;
                }
                ...
                box.RedirectUrl = GetRedirectUrlAfterLogin( returnUrl );
                return;
            }
        ...
        private void RedirectToSingleExternalAuthProviderIfNeeded( LoginInitializationBox box, List<NamedComponent<AuthenticationComponent>> externalAuthProviders )
        {
                 ...
                 && GetAttributeValue( AttributeKey.RedirectToSingleExternalAuthProvider ).AsBoolean() )
            {
                ...
                var authLoginUri = singleAuthProvider.Component.GenerateLoginUrl( GetCurrentPageUrl(), GetRedirectUrlAfterLogin() ).AbsoluteUri;
        ...
        private void SendConfirmation( UserLogin userLogin )
        {
            try
            {
                ...
                var mergeFields = GetMergeFields( new Dictionary<string, object>
                {
                    { "ConfirmAccountUrl", PathHelper.GetRootPath() + url.TrimStart( '/' ) },
                ...
                message.AppRoot = "/";
                message.ThemeRoot = PathHelper.ResolveRockUrl( "~~/", PageCache.SiteTheme );
